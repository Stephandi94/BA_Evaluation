...
def main():
    """
    comparison of the different rain rates
    """
    sub_dir = [29, 44, 59, 74, 89, 100]
    tick_labels = [0.3, 1.2, 5.0, 21.0, 91.5, 150.0]
    rain_augmentation = np.zeros((len(sub_dir), 2))
    rain_augmentation_physical = np.zeros((len(sub_dir), 2))
    for i, s in enumerate(sub_dir):
        rain_augmentation[i, :] = read_augmentation(os.path.join(r"Weather_Rain\{}\augmentation_stats.csv".format(s))) #TODO
        rain_augmentation_physical[i, :] = read_augmentation_phys(os.path.join(r"Weather_Rain\{}\rain_augmentation_stats_physical_{}.csv".format(s, s))) #TODO

    # only model values
    vis_aug = ev.VisEval()
    vis_aug.set_x_data([np.arange(len(sub_dir))])
    vis_aug.set_y_data([rain_augmentation[:, 0], rain_augmentation[:, 1]])
    ...
    vis_aug.to_file(os.path.join(r"Weather_Rain"), r"rain_augmentation_comparison") #TODO
    vis_aug.line_plot()

    # compared to phys model
    vis_aug_c = ev.VisEval()
    vis_aug_c.set_x_data([np.arange(len(sub_dir))])
    vis_aug_c.set_y_data([rain_augmentation, rain_augmentation_physical])
    ...
    vis_aug_c.to_file(os.path.join(r"Weather_Rain"), r"rain_augmentation_comparison_physical") #TODO
    ...
    rain_dist_int_phys = np.zeros((len(tick_labels), 6))
    ...
    clear_dist_phys = ed.read_csv_to_array(os.path.join(r"Weather_Rain/clean_distance_stats_physical.csv")).reshape(-1) #TODO
    clear_int_phys = ed.read_csv_to_array(os.path.join(r"Weather_Rain/clean_intensity_stats_physical.csv")).reshape(-1) #TODO
    rain_dist_int_phys[0, :] = np.array(np.concatenate((clear_dist_phys, clear_int_phys)))
    for i, l in enumerate(sub_dir):
        ...
        rain_dist_int[i+1, :] = np.array([rain_stats[1, 0], rain_stats[2, 1], rain_stats[3, 2], rain_stats[4, 0], rain_stats[5, 1], rain_stats[6, 2]]) # distance max_max, intensity mean_mean
        phys_dist = ed.read_csv_to_array(os.path.join(r"Weather_Rain/{}/rain_distance_stats_physical_{}.csv".format(l, l))).reshape(-1) #TODO
        phys_int = ed.read_csv_to_array(os.path.join(r"Weather_Rain/{}/rain_intensity_stats_physical_{}.csv".format(l, l))).reshape(-1) #TODO
        rain_dist_int_phys[i+1, :] = np.array(np.concatenate((phys_dist, phys_int)))
    ...
    data_dist[:, 0] = rain_dist_int[:, 2]
    data_dist[:, 1] = rain_dist_int_phys[:, 2]
    data_int[:, 0] = rain_dist_int[:, 3]
    data_int[:, 1] = rain_dist_int_phys[:, 3]
    ...
    vis_att_c.to_file(os.path.join(r"Weather_Rain"), r"rain_attenuation_comparison_physical") #TODO
    ...
    vis_int.set_y_data([rain_dist_int[:, 3:]])
    ...
    vis_int.set_legend_location((0.8, 0.1))
    vis_int.set_marker('.')
    vis_int.set_xlim([-0.1, len(tick_labels)-0.9])
    vis_int.to_file(os.path.join(r"Weather_Rain"), r"rain_intensities_comparison") #TODO
    vis_int.plot_stats()

    # just distances
    vis_dist = ev.VisEval()
    vis_dist.set_x_data(np.arange(len(tick_labels)))
    vis_dist.set_y_data([rain_dist_int[:, :3]])
    ...
    vis_dist.to_file(os.path.join(r"Weather_Rain"), r"rain_distances_comparison") #TODO
    vis_dist.plot_stats()
