...
def main():
    """
    evaluation of snow at 91.5 mm/h (89% in CARLA)
    """
    sub_dir = 89 # TODO ANPASSEN
    dest_path = os.path.join(r"Weather_Snow/{}".format(89)) # TODO ANPASSEN
    ...
    intensity_vis.set_data_labels(["91.5 mm/h snow", "no snow"]) # TODO REGENRATE ANPASSEN
    ...
    aug_vis.set_y_ticklabel(np.arange(
        (np.max(augmentation_stats_per_pcl[:, 2]) + 163) * -1,
        np.max(augmentation_stats_per_pcl[:, 1]) + 1282, 500
    ))
    ...
    src_phys_snow = os.path.join(r"..\Datasets\BA_AdverseWeather\data\VLP16_snow_91.5.bin") # TODO REGENRATE ANPASSEN
    ...
    phys_int.set_data_labels(["91.5 mm/h snow", "no snow"])  # TODO REGENRATE ANPASSEN
