...
def main():
    """
    evaluation of snow at 5.0 mm/h (59% in CARLA)
    """
    sub_dir = 59 # TODO ANPASSEN
    dest_path = os.path.join(r"Weather_Snow/{}".format(59)) # TODO ANPASSEN
    ...
    intensity_vis.set_data_labels(["5.0 mm/h snow", "no snow"]) # TODO REGENRATE ANPASSEN
    ...
    aug_vis.set_y_ticklabel(np.arange(
        (np.max(augmentation_stats_per_pcl[:, 2]) + 16) * -1,
        np.max(augmentation_stats_per_pcl[:, 1]) + 127, 50
    ))
    ...
    src_phys_snow = os.path.join(r"..\Datasets\BA_AdverseWeather\data\VLP16_snow_5.0.bin") # TODO REGENRATE ANPASSEN
    ...
    phys_int.set_data_labels(["5.0 mm/h snow", "no snow"])  # TODO REGENRATE ANPASSEN
