...
def main():
    """
    evaluation of snow at 150.0 mm/h (100% in CARLA)
    """
    sub_dir = 100 # TODO ANPASSEN
    dest_path = os.path.join(r"Weather_Snow/{}".format(100)) # TODO ANPASSEN
    ...
    intensity_vis.set_data_labels(["150.0 mm/h snow", "no snow"]) # TODO REGENRATE ANPASSEN
    ...
    aug_vis.set_y_ticklabel(np.arange(
        (np.max(augmentation_stats_per_pcl[:, 2]) + 277) * -1,
        np.max(augmentation_stats_per_pcl[:, 1]) + 1986, 500
    ))
    ...
    src_phys_snow = os.path.join(r"..\Datasets\BA_AdverseWeather\data\VLP16_snow_150.0.bin") # TODO REGENRATE ANPASSEN
    ...
    phys_int.set_data_labels(["150.0 mm/h snow", "no snow"])  # TODO REGENRATE ANPASSEN
