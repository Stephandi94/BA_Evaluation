...
def main():
    """
    evaluation of rain at 0.3 mm/h (29% in CARLA)
    """
    sub_dir = 29 # TODO ANPASSEN
    dest_path = os.path.join(r"Weather_Rain/{}".format(29)) # TODO ANPASSEN
    ...
    intensity_vis.set_data_labels(["0.3 mm/h rain", "no rain"]) # TODO REGENRATE ANPASSEN
    ...
    src_phys_rain = os.path.join(r"..\Datasets\BA_AdverseWeather\data\VLP16_rain_0.3.bin") # TODO REGENRATE ANPASSEN
    ...
    ed.stats_1x3_to_csv(phys_gt_distance_stats, os.path.join(r"Weather_Rain"), "clean_distance_stats_physical")
    ed.stats_1x3_to_csv(phys_gt_intensity_stats, os.path.join(r"Weather_Rain"), "clean_intensity_stats_physical")
    ...
    phys_int.set_data_labels(["0.3 mm/h rain", "no rain"])  # TODO REGENRATE ANPASSEN
    ...
    phys_dist = ev.VisEval()
    phys_dist.set_y_data([phys_distance, phys_distance_gt])
    phys_dist.set_x_label("distance [m]")
    phys_dist.set_y_label("frequency")
    phys_dist.set_data_labels(["0.3 mm/h rain", "no rain"])  # TODO REGENRATE ANPASSEN
    phys_dist.define_bins(int(np.max(phys_distance_gt)), 0, int(np.max(phys_distance_gt)))
    phys_dist.set_legend_location('upper right')
    phys_dist.set_xlim([0, 102])
    phys_dist.to_file(dest_path, "phys_rain_distances")
    phys_dist.histogram_comparison()
