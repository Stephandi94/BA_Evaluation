...
def main():
    """
    evaluation of snow at 1.2 mm/h (44% in CARLA)
    """
    sub_dir = 44  # TODO ANPASSEN
    dest_path = os.path.join(r"Weather_Snow/{}".format(44))  # TODO ANPASSEN
    ...
    if os.path.exists(os.path.join(r"Weather_Snow/{}/augmentation_per_pcl.csv".format(sub_dir))):
        ...
        augmentation_stats_per_pcl = ed.read_csv_to_array(
            os.path.join(r"Weather_Snow/{}/augmentation_per_pcl.csv".format(sub_dir)), True)
    ...
    intensity_vis.set_data_labels(["1.2 mm/h snow", "no snow"])  # TODO REGENRATE ANPASSEN
    ...
    aug_vis.set_y_ticklabel(np.arange(
        (np.max(augmentation_stats_per_pcl[:, 2]) + 17) * -1,
        np.max(augmentation_stats_per_pcl[:, 1]) + 41, 25
    ))
    ...
    src_phys_snow = os.path.join(r"..\Datasets\BA_AdverseWeather\data\VLP16_snow_1.2.bin")  # TODO REGENRATE ANPASSEN
    ...
    ed.stats_1x3_to_csv(phys_augmentation, dest_path, "snow_augmentation_stats_physical_{}".format(sub_dir),
                        ["similar", "lost", "noise"])
    ...
    phys_int.set_data_labels(["1.2 mm/h snow", "no snow"])  # TODO REGENRATE ANPASSEN
