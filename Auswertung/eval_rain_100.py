...
def main():
    """
    evaluation of rain at 150.0 mm/h (100% in CARLA)
    """
    sub_dir = 100 # TODO ANPASSEN
    dest_path = os.path.join(r"Weather_Rain/{}".format(100)) # TODO ANPASSEN
    ...
    intensity_vis.set_data_labels(["150.0 mm/h rain", "no rain"]) # TODO REGENRATE ANPASSEN
    ...
    aug_vis.set_y_ticklabel(np.arange(
        (np.max(augmentation_stats_per_pcl[:, 2]) + 52) * -1,
        np.max(augmentation_stats_per_pcl[:, 1]) + 100, 100
    ))
    ...
    src_phys_rain = os.path.join(r"..\Datasets\BA_AdverseWeather\data\VLP16_rain_150.0.bin") # TODO REGENRATE ANPASSEN
    ...
    phys_int.set_data_labels(["150.0 mm/h rain", "no rain"])  # TODO REGENRATE ANPASSEN
